...
void AWP(){
    ...
    wait(1000, msec);
    MoveEncoderPID(TestPara, -100, 3, 0.3, 57, true);
    TurnMaxTimePID(TestPara, -87, 0.5, true);
    ...
    wait(200,msec);
    MoveEncoderPID(TestPara, 100, 0.2, 0.4,-87, true); //move back to let goal sit in
    Tilt.set(false); // tilt mogo
    RunRoller(-100); //run intake
    TurnMaxTimePID(TestPara, 107, 0.8,true);
    MoveEncoderPID(TestPara, -100, 23, 0.4, 107, true); //move back to let goal sit in
