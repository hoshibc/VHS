...
void redAWP(){
    ...
    MoveEncoderPID(TestPara, 75, 7.5, 0.5, -140, true);
   
    TurnMaxTimePID(TestPara, -60, 0.6, true);
    MoveEncoderPID(TestPara, 70, 10, 0.5, -60, true);
    wait(100,msec);
    RunRoller(-100); //score onto alliance stake
    wait(1450, msec); //wait for scoring both rings onto alliance stake
    RunRoller(0); //run intake
    MoveEncoderPID(TestPara, -90, 6.5, 0.3, -60, true);// delta angle needs to be 58(ik its weird)
    TurnMaxTimePID(TestPara, 70, .8, true);
    Tilt.set(true); // down to grab mogo

    MoveEncoderPID(TestPara, 100, 30, 0.5, 70, false);
    MoveEncoderPID(TestPara, 70,10,0.3,70,true);
    MoveEncoderPID(TestPara, 40, 5, 0.3, 70, true); //slow down to grab mogo
    Clamp.set(true); //close clamp 
    wait(380,msec);
    MoveEncoderPID(TestPara, 100, 1.5, 0.4, 70, true); //move back to let goal sit in
    Tilt.set(false); // tilt mogo
   // wait(380,msec);

    TurnMaxTimePID(TestPara,-105, 0.8,true); 
    RunRoller(-100); //run intake
    //wait(380,msec);
    MoveEncoderPID(TestPara, -100, 30, 0.4, -105, true); //grab ring 2
    TurnMaxTimePID(TestPara, -145, 0.4,true);
    MoveEncoderPID(TestPara, -85, 13, 0.4, -145, true); //grab ring 3
    TurnMaxTimePID(TestPara, 50, 0.7, true);
